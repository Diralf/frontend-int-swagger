...
module.exports = {
  ...
  create(context) {
    ...
    const getInterfaceFromSwaggerJson = (interfaceName, swaggerDefinitions) => {
      const definitionName = interfaceName.replace(/Dto$/, '');
      return swaggerDefinitions[definitionName];
    }
    ...
    return {
      TSInterfaceDeclaration(node) {
        ...
        const interfaceSchema = getInterfaceFromSwaggerJson(interfaceName, definitions);
        if (!interfaceSchema) {
